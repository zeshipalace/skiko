...
- 补丁栈(在基点之上):
  ...
  - Windows Direct3D 闲置纹理回收(voxzen.20):`skiko.gpu.resourceCacheLimit` 只会在缓存超过字节上限后执行 LRU 淘汰,低于上限但已经离屏的封面纹理和动态位图旧代际会继续占用 GPU/堆外内存。新增可选的 `skiko.gpu.resourceCacheCleanup.maxUnusedMillis`;启用后由单个低优先级守护线程定期调用 Skia `GrDirectContext::performDeferredCleanup`,只回收已解锁且超过指定时长未使用的资源,当前帧和可见列表工作集不受影响;扫描与绘制、缩放和销毁共用 `drawLock`,避免跨线程访问 `GrDirectContext`。`skiko.gpu.resourceCacheCleanup.intervalMillis` 控制扫描间隔(默认 1000ms),`skiko.gpu.resourceCacheCleanup.log=true` 可输出每次实际释放量;未设置 maxUnusedMillis 时完全保持上游缓存行为。实现仅增加 Direct3D 内部字段与 private JNI 入口,不改变任何既有公开类、构造器或方法签名
