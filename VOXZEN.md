...
- 补丁栈(在基点之上):
  ...
  - Linux 顶层 redrawer 还原为 `LinuxOpenGLRedrawer`(voxzen.13):上游 #1234 把各平台 redrawer 统一包进 `OnScreenRedrawer`,`SkiaLayer.redrawer` 的类名随之改变,mediamp(`SkiaOpenGLInterop`)按 0.9.37.4 布局反射 `LinuxOpenGLRedrawer` 类名与 `contextHandler`/`context` 字段全部失败,MV 黑屏抛 `IllegalStateException`。`LinuxOpenGLRedrawer` 改为自身实现 `Redrawer` 并在 `Actuals` 的 Linux OpenGL 路径直接作为顶层 redrawer 返回,帧循环完整委托给内部持有的 `OnScreenRedrawer`,渲染行为不变;新增 `org.jetbrains.skiko.context.ContextHandler`/`OpenGLContextHandler` 兼容 shim,`AbstractOpenGLRedrawer` 通过 `onGlContextChanged` 钩子把 `DirectContext` 同步进 shim。注意 `setVisible` 与 `AWTRedrawer` 同签名成员构成继承冲突,必须显式 `super` 消歧,且不能委托给 `OnScreenRedrawer`(其 `setVisible` 回调 `renderer.setVisible` 会形成无限递归)
