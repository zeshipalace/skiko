...
- 补丁栈(在基点之上):
  ...
  - Windows Direct3D 快速缩放无残影(voxzen.16):对客户区填满窗口的自定义装饰窗口,在更早的 `WM_WINDOWPOSCHANGING` 阶段完成布局、`ResizeBuffers` 与 `Present`,避免新窗口边界先于 DirectX 帧被观察到;透明 DirectComposition 交换链在每个同步 resize frame 的 `Present` 后等待 frame-latency object,确认 DXGI 已消费当前帧后再以 `DwmFlush` 提交窗口边界;实时缩放期间对子 HWND 使用 `SWP_NOCOPYBITS | SWP_NOREDRAW`,并从 `WM_NCCALCSIZE` 返回 `WVR_REDRAW`,禁止 Win32/AWT 把旧客户区像素复制到新尺寸或插入中间背景擦除,消除快速放大边缘白线与缩放残影
