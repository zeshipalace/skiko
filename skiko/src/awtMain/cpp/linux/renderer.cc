...
extern "C"
{
    ...
    // GLX contexts can only be current on one thread at a time, so concurrent presentation releases
    // the dispatcher-thread binding before handing each context to a worker
    JNIEXPORT void JNICALL Java_org_jetbrains_skiko_renderer_LinuxOpenGLRendererKt_releaseCurrent(JNIEnv *env, jobject renderer, jlong displayPtr)
    {
        Display *display = fromJavaPointer<Display *>(displayPtr);
    ...
    JNIEXPORT jlong JNICALL Java_org_jetbrains_skiko_renderer_LinuxOpenGLRendererKt_createContext(JNIEnv *env, jobject renderer, jlong displayPtr, jlong windowPtr, jboolean transparency)
    {
        Display *display = fromJavaPointer<Display *>(displayPtr);
        Window window = fromJavaPointer<Window>(windowPtr);
        if (!display) return 0;
