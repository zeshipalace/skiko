 ...
 * correctness. Two displays running at the same refresh rate share the first
 * CRTC that matches: the tick rate is right for both, but their phases can
 * differ by up to one frame. And when the toolkit reports no refresh rate at
 * all, there is nothing to match on, so the first active CRTC is used — right
 * on a single-display desktop, a guess on a mixed-refresh one. (Binding by
 * compositor connector name would be exact, but needs toolkit-internal data
 * Skiko cannot reach; the connectorName parameter is reserved for that and is
 * currently always null.) Device access relies on the
