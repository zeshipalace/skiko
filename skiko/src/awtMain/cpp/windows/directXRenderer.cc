...
static void javaDrawFrameWhileLiveResizing(LiveResizeState *s, bool isResizeFrame) {
    ...
    if (mid) env->CallVoidMethod(s->renderer, mid, (jint)s->lastFrameClientSize.cx, (jint)s->lastFrameClientSize.cy, (jboolean)isResizeFrame);
    if (env->ExceptionCheck()) { env->ExceptionDescribe(); env->ExceptionClear(); }
}
...
extern "C"
{
    ...
    bool isAdapterSupported(JNIEnv *env, jobject renderer, IDXGIAdapter1 *hardwareAdapter) {
        ...
        static jclass cls = (jclass) env->NewGlobalRef(env->FindClass("org/jetbrains/skiko/renderer/Direct3DRenderer"));
    ...
    JNIEXPORT void JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_initSwapChain(
        JNIEnv *env, jobject renderer, jlong devicePtr, jint width, jint height, jboolean transparency, jboolean synchronousLiveResize)
    {
        __try
        {
    ...
    JNIEXPORT void JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_resizeBuffers(
    ...
    JNIEXPORT void JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_swap(
        JNIEnv *env, jobject renderer, jlong devicePtr, jboolean isVsyncEnabled)
    {
        __try
        {
            DirectXDevice *d3dDevice = fromJavaPointer<DirectXDevice *>(devicePtr);
    ...
    JNIEXPORT void JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_disposeDevice(
        JNIEnv *env, jobject renderer, jlong devicePtr)
    {
        DirectXDevice *d3dDevice = fromJavaPointer<DirectXDevice *>(devicePtr);
        delete d3dDevice;
    ...
    JNIEXPORT jint JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_getBufferIndex(
        JNIEnv *env, jobject renderer, jlong devicePtr)
    {
        __try {
            DirectXDevice *d3dDevice = fromJavaPointer<DirectXDevice *>(devicePtr);
    ...
    JNIEXPORT jstring JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_getAdapterName(JNIEnv *env, jobject renderer, jlong adapterPtr)
    {
    ...
    JNIEXPORT jlong JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_getAdapterMemorySize(JNIEnv *env, jobject renderer, jlong adapterPtr)
    {
        IDXGIAdapter1 *adapter = fromJavaPointer<IDXGIAdapter1 *>(adapterPtr);
    ...
    JNIEXPORT void JNICALL Java_org_jetbrains_skiko_renderer_Direct3DRenderer_uninstallLiveResizeHook(
        JNIEnv *env, jobject renderer, jlong handle)
    {
        ...
        if (state->renderer) env->DeleteGlobalRef(state->renderer);
