...
- 补丁栈(在基点之上):
  ...
  - Windows Direct3D 低延迟同步缩放(voxzen.6):同步缩放交换链使用 `DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT` 并将最大帧延迟设为 1,防止高频拖拽产生的 `Present` 在 DirectComposition 中积压多帧;实时缩放帧先 `Present`,再用 `DwmFlush` 等 DWM 接收匹配的表面更新,最后返回 `WM_NCCALCSIZE` 提交窗口几何,避免窗口边缘领先内容一个合成步;透明 DirectComposition/Acrylic 路径不变
